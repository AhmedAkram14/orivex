...
Implementation note (Production Readiness Audit): this document remains the authoritative, hand-written contract for design discussion and covers the full envisioned V1 surface (28 resources, cursor pagination, HATEOAS-lite links, etc). What's actually implemented today is a narrower, offset-paginated subset (see docs/15-observability.md and the codebase's per-module controllers for current reality). A real, generated OpenAPI document now exists alongside this one — mounted live at `GET /docs` (Swagger UI, non-production by default; set `OPENAPI_ENABLED=true` to expose it in production) and exportable via `pnpm --filter @orivex/backend openapi:generate` (writes `apps/backend/openapi.json`), built directly from the running NestJS controllers/DTOs via `@nestjs/swagger`. Treat the generated document as the source of truth for "what's actually callable today"; treat this file as the source of truth for "what the platform is designed to become." Reconciling the two fully (annotating every DTO with `@ApiProperty` for rich generated descriptions, converting cursor-based pagination from offset) is intentionally out of scope for this pass.
