...
- ✅ **Implemented** — the login form reacts to every mock error outcome distinctly: Account Locked routes to `/account-locked` (built in the next milestone), Email Not Verified shows an inline resend-verification action, Too Many Attempts and Invalid Credentials surface inline
- 📋 **Remaining** — Account Locked, Session Expired, Unauthorized, Forbidden, and Access Denied pages; Device Sessions and Login History UI; Security Center
