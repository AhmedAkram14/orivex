...
| 8 | Patient Portal | P1 | 🚧 | Phase 6 | Patient, Clinical — workspace architecture implemented against no business modules; real profile/appointments/records/prescriptions/vitals data blocked until those backend modules are wired in |
| 9 | Appointment System & Calendar (incl. Queue Management) | P1 | 🚧 | Phase 6 | Scheduling, Consultation — scheduling/booking architecture implemented against no real SchedulingModule integration; Queue Management sub-scope 🔒 blocked — no Queue module |
...
- ✅ **Implemented** — Doctor Profile (`app/[locale]/(protected)/doctor/profile/page.tsx`): View mode (`DoctorProfileView` — avatar, Professional Information, Qualifications, Experience, Availability summary, Languages, Contact Information, every section the phase's scope names) and Edit architecture (`DoctorProfileForm`, React Hook Form + Zod, editing only the fields `DoctorProfileUpdateRequest` allows — identity and verification-backed fields are deliberately excluded, `TrustModule`'s own remaining scope). Read-only mode is the same `DoctorProfileView` component with no separate variant to keep in sync — a viewer without edit access simply never sees the Edit action, per that component's own comment. Backed by a real (mocked) GET/PATCH `/doctor/profile` pair; the mock profile is realistic administrative data (name, specialization, qualifications), not clinical patient data, so it doesn't fall under the "no fake medical data" restriction the dashboard's summary/upcoming-work endpoints do
- ✅ **Implemented** — Doctor Workspace nav restructured into a group (Overview + Profile children) so both routes get breadcrumb support from the same `NAVIGATION_CONFIG` source of truth
- ✅ **Implemented** — Schedule Foundation (`app/[locale]/(protected)/doctor/schedule/page.tsx`): reusable scheduling primitives (`shared/ui/schedule/`) — `CalendarHeader`, `DateNavigation`, `WeeklyCalendar`, `DailyTimeline`, `AvailabilityBlock`, `TimeSlot` — composed into a week grid (real recurring availability, backed by a new mocked `/doctor/availability` endpoint) with a drill-down daily timeline marking each business hour `available` or `blocked`. Deliberately no `booked` status and no click-to-book behavior: this is the scheduling *architecture* only, not appointment logic, per this phase's explicit scope. Date math lives in a pure, unit-testable `features/doctor/lib/week.ts` rather than inline in the page
- ✅ **Implemented** — Patient Queue architecture (`app/[locale]/(protected)/doctor/queue/page.tsx`): reusable queue primitives (`shared/ui/queue/`) — `PatientQueueCard`, `QueueStatus`, `EstimatedWaitTime`, `QueueFilters`, `WaitingQueue`, `CurrentPatientCard` — composed with a real (mocked) `/doctor/queue` endpoint. Honestly empty today (no Patient/Appointment module exists to produce real entries), with entry labels designed to be anonymized placeholders ("Patient #3") rather than fabricated names whenever real data does arrive
- ✅ **Implemented** — Consultation Workspace layout (`app/[locale]/(protected)/doctor/consultation/page.tsx`): the reusable three-pane `ConsultationContainer` (`shared/ui/consultation/`) — Left Navigation, Main Workspace, Right Information Panel, stacking vertically on small viewports rather than hiding panes — plus the generic `EmptyWorkspace` primitive (`shared/ui/layout/`) filling every pane today. Placeholder containers only, exactly as scoped: selecting a left-nav section only changes which empty-state placeholder shows, no medical form or real consultation/patient data exists yet
- ✅ **Implemented** — Workspace component consolidation: `ScheduleCard` (`shared/ui/schedule/`, the last of this milestone's named components) plus a real usage — `NextAvailabilityCard` on the Doctor Dashboard, deriving the next upcoming availability block from the same real (mocked) weekly-availability data the Schedule Foundation uses (`features/doctor/lib/week.ts`'s `getNextAvailability`), honestly empty when none is configured. All seven named reusable components (`DoctorStatCard`, `PatientQueueCard`, `ScheduleCard`, `QuickActionCard`, `EmptyWorkspace`, `TimelineCard`, `ConsultationContainer`) are now built, with no duplication across them (`QuickActionCard`/`QuickActions` share one implementation since Phase 6)
- ✅ **Implemented** — accessibility verification pass across every Doctor Portal component, catching and fixing three real issues: (1) `ConsultationContainer`'s main pane used a `<main>` element, which always renders inside `AppShell`'s own `<Content>` `<main>` — a duplicate landmark, now a plain `<div>`; (2) several new components (`ScheduleCard`, `TimeSlot`, `DoctorStatCard`, `QuickActionCard`) plus the pre-existing `NavItem`/`NavGroup` and Phase 6's `CommandPalette`/`NotificationBell`/`NotificationPanel` used bare `transition-colors`/`transition-transform` instead of the design system's reduced-motion-aware `duration-(--duration-fast)` token, silently failing to respect `prefers-reduced-motion` — now all fixed to the same convention; (3) `WeeklyCalendar`'s "today" highlight was color-only with no screen-reader announcement, now takes an optional `todayAnnouncement` string rendered via `sr-only`. RTL, keyboard navigation (native buttons/links throughout, no custom roving-tabindex), and theme compatibility (design-token classes throughout, no hardcoded colors) were audited with no further issues found
- ✅ **Implemented** — comprehensive test suite (Milestone 8): 47 new tests across 17 files — pure date-math (`week.test.ts`), every reusable schedule/queue/consultation primitive, `RequireRole` (previously untested), and all five Doctor Portal pages (dashboard, profile, schedule, queue, consultation) rendered against the real MSW mock backend with a `doctor`-role `AuthContext`. Storybook stories added for every new `shared/ui/schedule`, `shared/ui/queue`, `shared/ui/consultation`, and remaining `shared/ui/layout` primitive (`DoctorStatCard`, `TimelineCard`, `QuickActionCard`, `EmptyWorkspace`). Writing these tests caught and fixed two more real bugs: a missing `doctor.profile.specialization` translation key (the field's label silently fell back to next-intl's raw-key placeholder) and a significant breadcrumb bug — `findTrail` picked the *first* nav-config entry whose `href` string-prefixed the current path, so every Doctor Portal sub-page (`/doctor/profile`, `/doctor/schedule`, etc.) incorrectly matched the shorter sibling route `/doctor` (Overview) instead of itself; fixed to pick the *longest* (most specific) match among all candidates, with a dedicated regression test. Also fixed a project-wide `next-intl` `ENVIRONMENT_FALLBACK` timezone warning (missing global `timeZone` config) that every `useFormatter().dateTime()` call was silently triggering, both in tests and in real production rendering — now configured once in `shared/i18n/request.ts` (`Africa/Cairo`, Orivex Egypt V1's operating timezone), Storybook's preview, and the shared test-render helper
- 📋 **Remaining** — none for this phase's originally scoped Doctor Workspace architecture
...
**Priority:** P1 · **Status:** 🚧 In Progress (workspace architecture, no clinical business logic yet) · **Depends on:** Phase 6 · **Backend dependency:** `PatientModule`, `ClinicalModule` (read)
...
**Priority-change note (per Section 0's rule):** pulled forward from 📋 Planned to active implementation on explicit architect direction, scoped deliberately to *Patient Workspace architecture only* — no real Patient/Clinical business logic is built here, mirroring Phase 7's own precedent. This phase builds the dashboard, profile, appointments, medical-records, prescriptions, and health-dashboard *architecture* those future backend modules will render inside.

**Scope pulled forward from later phases, on the same explicit-direction basis as this phase's own pull-forward from Section 0:**
- Milestone 4 (Medical Records) and part of Milestone 6 (Health Dashboard's vitals trend charts) pull forward a slice of **Phase 10**'s Medical Timeline / Health Journey scope, onto a simpler, self-contained `MedicalRecordEntry`/`VitalReading` shape rather than Phase 10's real `HealthGraphNode`/`HealthJourney` data model. This is a data-source swap when Phase 10 lands for real, not a UI rewrite — the same posture Phase 7's own Schedule/Queue architecture already took relative to Phase 9.
- The Insurance section (Milestone 2, Patient Profile) was **not** pulled forward — it stays deferred to Phase 16 exactly as this phase's own Scope line above states, rendered today as an honest "not yet available" placeholder (`EmptyState`) rather than built ahead of a billing/payment backend.

**Implementation checklist:**
- ✅ **Implemented** — Milestone 1, Patient Dashboard (`app/[locale]/(protected)/patient/page.tsx`): `WelcomeHeader`, `HealthSummary` (real zero-state counts via the renamed `LinkableStatCard` — previously `DoctorStatCard`, generalized once a second, non-doctor caller needed it, no duplicate component created), `UpcomingAppointmentsWidget`, `ActivePrescriptionsWidget`, `PatientQuickActions`. Backed by `features/patient/` (mirrors `features/doctor/`'s shape) and a real MSW mock domain (`mocks/patient-store.ts` + `mocks/handlers/patient.ts`) that honestly returns zero counts / empty lists, since no Scheduling/Clinical module exists yet. New `patient@orivex.dev` demo account and `patient-workspace` nav group (`roles: ['patient']`) added
- ✅ **Implemented** — Milestone 2, Patient Profile (`app/[locale]/(protected)/patient/profile/page.tsx`): View mode (`PatientProfileView` — avatar, Personal Information, Medical Information, Emergency Contacts, Insurance placeholder, Settings placeholder) and Edit architecture (`PatientProfileForm`, React Hook Form + Zod, `useFieldArray` for up to 5 emergency contacts). Medical Information (blood type/allergies/chronic conditions) is deliberately **always read-only** — CLAUDE.md's "AI never writes directly to clinical records" extended here to "a patient never self-edits their own clinical record" either, since that data is clinician-entered. Backed by a real (mocked) GET/PATCH `/patient/profile` pair; profile is a believable administrative seed (matching the demo account), `medicalInfo` stays an honest empty/undefined state
- ✅ **Implemented** — Milestone 3, Appointments (`app/[locale]/(protected)/patient/appointments/page.tsx`): calendar foundation (`AppointmentsCalendar`, reusing Doctor Portal's `WeeklyCalendar`/`DateNavigation` primitives, read-only day markers — no booking, that's Phase 9's scope), Upcoming/History tabs with a History `FilterTabs` (All/Completed/Cancelled). New reusable `shared/ui/appointments/AppointmentCard` deliberately viewpoint-agnostic (`counterpartyName`, not `doctorName`) so a future Doctor/Admin appointment list can reuse it unchanged. Two extractions during this milestone: `shared/ui/queue/QueueFilters` refactored to a thin wrapper over a new generic `shared/ui/filter-tabs.tsx`; `features/doctor/lib/week.ts`'s pure date functions moved to `shared/lib/date/week.ts` with a backward-compatible re-export, since Patient Portal's calendar needed the same week math Doctor Portal's Schedule Foundation already built
- ✅ **Implemented** — Milestone 4, Medical Records (`app/[locale]/(protected)/patient/records/page.tsx`) — see the pulled-forward-scope note above. Type-filtered (`FilterTabs`: All/Visits/Diagnoses/Allergies/Conditions) chronological timeline via new `shared/ui/timeline/RecordTimelineEntry` and a real, never-faked `RecordDownloadButton` (a genuine `<a href download>`, architecturally complete but only ever rendered when a record actually carries a resolved `downloadUrl`), plus honest "not yet available" Lab Results/Imaging placeholder sections
- ✅ **Implemented** — Milestone 5, Prescriptions (`app/[locale]/(protected)/patient/prescriptions/page.tsx`): Active/Previous tabs via new reusable `shared/ui/medications/MedicationCard` and `DosageVisualization` (a real pill-dot dose-count indicator, not a fabricated adherence chart). Refill-status badge shown only for `due-soon`/`due`/`none-remaining` states, keeping an unremarkable `not-due` state visually quiet
- ✅ **Implemented** — Milestone 6, Health Dashboard (`app/[locale]/(protected)/patient/health/page.tsx`) — see the pulled-forward-scope note above. Weight/Blood Pressure/Blood Sugar vitals, each a reusable `shared/ui/health/VitalTrendCard` backed by a new dependency-free SVG `TrendChart` sparkline — this codebase's "charts architecture" primitive, built without introducing a charting-library dependency (none exists elsewhere in the project, and CLAUDE.md requires discussion before adding a new framework)
- ✅ **Implemented** — Milestone 7, accessibility and RTL verification pass across every Patient Portal component from Milestones 1–6, catching and fixing two real issues: (1) the Medical Records timeline rendered as a plain `<div>` list, unlike Appointments'/Prescriptions' `<ul>/<li>` — screen readers had no way to announce item count/position for a chronological record list, now `<ol>/<li>`; (2) `TrendChart`'s hand-rolled SVG sparkline plots left-to-right regardless of document direction (an SVG's own coordinate system doesn't inherit `dir` the way `WeeklyCalendar`'s CSS grid auto-mirrors), so under Arabic RTL the chart read newest-to-oldest — now mirrors via `rtl:-scale-x-100`, following the `Icon` component's existing `flipRtl` policy for directional content. Keyboard navigation, focus management, and reduced-motion were confirmed already compliant project-wide (Radix `Tabs`, `Icon`'s `flipRtl`, the centralized reduced-motion token convention Phase 7 established) with no further changes needed. Full en/ar translation-key parity verified programmatically (132 keys each, zero missing either direction)
- ✅ **Implemented** — Milestone 8, comprehensive test suite: 36 new tests across 15 new test files this phase added (on top of the pre-existing suite, plus `linkable-stat-card.test.tsx` carried over unchanged from its M1 rename), covering every new reusable `shared/ui` primitive (`AppointmentCard`, `RecordTimelineEntry`, `RecordDownloadButton`, `MedicationCard`, `DosageVisualization`, `TrendChart`, `VitalTrendCard`, `FilterTabs`), the extracted `shared/lib/date/week.ts`, and all six Patient Portal pages (dashboard, profile, appointments, records, prescriptions, health) rendered against the real MSW mock backend with a `patient`-role `AuthContext`. Storybook stories added for every new reusable component — full coverage confirmed by directory audit, no gaps. Project-wide suite: 72 test files, 177 tests, all green
- 📋 **Remaining** — none for this phase's originally scoped Patient Workspace architecture
- 🔒 **Blocked** — real profile/appointment/medical-record/prescription/vitals data: `PatientModule`/`ClinicalModule` integration remains a backend dependency this phase's UI architecture doesn't require yet but will need before any of the above can show real data. Insurance/billing views remain deferred to Phase 16 as originally scoped.

---

### 3.10 Phase 9 — Appointment System & Calendar (incl. Queue Management)

**Priority:** P1 · **Status:** 🚧 In Progress (scheduling architecture built, no `SchedulingModule` backend integration yet; Queue Management sub-scope remains blocked) · **Depends on:** Phase 6 · **Backend dependency:** `SchedulingModule`, `ConsultationModule` (built); Queue Management sub-scope 🔒 blocked — no Queue module (Section 1.4)
...
**Priority-change note (per Section 0's rule):** pulled forward from 📋 Planned to active implementation on explicit architect direction, scoped as *Scheduling & Appointment Infrastructure architecture only* — no real `SchedulingModule` backend integration, no real multi-doctor directory, and Queue Management stays explicitly out of scope (still blocked on its own backend module, per Section 1.4). Mirrors Phase 7's and Phase 8's own precedent: build the real architecture against honest MSW-mocked data, not fake business logic.

**Implementation checklist:**
- ✅ **Implemented** — Milestone 1, Scheduling Foundation (`features/scheduling/`): the shared domain model (`RecurringWeeklySchedule`, `WorkingHoursDay`, `ScheduleException`, `TimeSlotData`, `SchedulingRules`) — richer than Phase 7's `AvailabilityBlockData` (hour-granularity, no breaks/exceptions), exactly the real replacement that type's own doc comment anticipated. Pure, unit-tested utilities: time-of-day minute math, slot generation respecting working hours/breaks/buffer, and a double-book-prevention conflict-detection function mirroring the check `SchedulingModule`'s real slot-reservation logic runs server-side (docs/10-backend-architecture.md). One real (mocked) `/scheduling/rules` endpoint
- ✅ **Implemented** — Milestone 2, Doctor Availability (`app/[locale]/(protected)/doctor/schedule/page.tsx`): retires Phase 7's read-only Schedule Foundation in favor of a real editing architecture — `WorkingHoursForm` (7-day recurring template, working-day toggle, hours, up to 5 breaks per day, RHF + Zod with cross-field break/overlap validation) and `ScheduleExceptionsManager` (vacation/unavailable/extra-hours date overrides), both backed by real (mocked) PATCH/POST/DELETE `/scheduling/*` endpoints. New `resolveDayForDate` util resolves the recurring template + any exception into one effective day, and a new `MonthCalendar` primitive (+ `shared/lib/date/month.ts`, mirroring the `week.ts` extraction) gives the page a real Month view alongside Week/Day
- ✅ **Implemented** — Milestone 3, enterprise calendar UI: new reusable `TimeGrid` (a real slot grid with Tooltip-based hover detail), `AgendaList` (a flat chronological view across a date range), and `Legend`. The Day tab now renders `TimeGrid` from real generated slots instead of a plain hour-row list; a new Agenda tab flattens the next two weeks' real generated slots through the same `resolveDayForDate`/`generateDaySlots` pipeline every other view uses, so no view ever disagrees with another about the same date
- ✅ **Implemented** — Milestone 4, Booking Architecture: a real, mocked booking round trip (select an available slot → `BookingSummaryCard` review → confirm → manage the resulting booking) against the doctor's real availability, scoped to "the current doctor" (no doctor directory exists yet). Real (mocked) POST/PATCH/DELETE `/scheduling/bookings` and `/scheduling/waitlist` endpoints, including genuine conflict detection — booking an already-booked slot returns a real 409, the "this slot was just taken" retry UX this phase's own scope line calls out by name, not a generic error. Confirmed bookings actually block those slots in every calendar view via a new `bookedRangesForDate` bridge util. Reschedule re-enters slot selection for the same booking; Cancel opens a confirm dialog. A date with no available slots offers `WaitlistJoinCard` instead of a dead end. New patient-facing entry point at `/patient/appointments/book`, linked from the existing Phase 8 Appointments page — deliberately **not** unified with that page's own `Appointment` list (the two domains don't share a backend today; documented honestly rather than faked together)
- ✅ **Implemented** — Milestone 5, remaining reusable components: `StatusBadge` (the single tone → `Badge`-variant mapping, extracted from `AgendaList`'s and `BookingSummaryCard`'s own duplicate maps), `AvailabilityCard` (a day's recurring-schedule summary row), `CalendarSidebar`, `EmptyCalendar` (shown when a doctor has configured zero working days at all), `LoadingCalendar`, and `ConflictIndicator` (built here, wired to real data in Milestone 6). A generic "Calendar" view-switcher wrapper and a separate "BookingCard" were deliberately **not** built — no second consumer exists to justify the former, and `BookingSummaryCard` already covers the latter (avoiding a near-duplicate, per this project's own "extract common logic instead of duplicating" rule)
- ✅ **Implemented** — Milestone 6, Scheduling Rules made real: Holiday support (a new global, doctor-agnostic `Holiday` type, seeded with real fixed-date Egyptian national holidays — deliberately not lunar-calendar ones like Eid, whose exact date isn't safe to assert without an authoritative source), wired into `resolveDayForDate` with a priority order (an explicit `extra-hours` exception always wins, then a holiday blocks the date, then a vacation/unavailable exception). Double-book prevention made real: the Booking Flow now calls `detectConflict` (built in Milestone 1, never wired to any UI until now) to enforce the minimum-notice and maximum-booking-window rules slot generation itself doesn't check, surfaced via `TimeGrid`'s hover detail and (for the currently selected slot, re-checked at confirm time) the `ConflictIndicator` primitive. Timezone awareness: the Doctor Schedule page now shows which timezone its times are displayed in
- ✅ **Implemented** — Milestone 7, accessibility audit across every component/page built in Milestones 1–6: found and fixed one real gap — `WorkingHoursForm`'s per-day and per-break time inputs had no accessible label (a screen reader announced only "time input" with no indication of which day/break), now each carries a real `aria-label`. Keyboard navigation, ARIA, focus management, reduced motion, RTL, and responsive layout were confirmed already compliant (native buttons throughout, Radix Dialog/Tabs/Select/Tooltip's built-in focus management, the project-wide reduced-motion token convention, logical direction classes). Full en/ar translation-key parity verified programmatically at every milestone (zero missing keys either direction)
- ✅ **Implemented** — Milestone 8, comprehensive test suite: Storybook stories for every new reusable `shared/ui/schedule` component (full coverage confirmed by directory audit — the only gaps are `AvailabilityBlock`/`CalendarHeader`/`DailyTimeline`, pre-existing Phase 7 components this phase reused as-is and didn't touch). `DailyTimeline` itself is now unused by any page (the Day view moved to `TimeGrid` in Milestone 3) but was deliberately kept, not deleted — a real, general-purpose single-day hour-by-hour primitive, not dead weight specific to this phase. `TooltipProvider` added to the shared `renderWithProviders` test helper so any component using `Tooltip` is covered by default. New integration tests added during this milestone closed real coverage gaps: `WorkingHoursForm`'s save flow (toggle a day off, save, confirm the read-only summary reflects it), `ScheduleExceptionsManager`'s add/remove round trip, and the Booking Flow's waiting-list path (previously only its "available slot" path was exercised). Project-wide suite: 91 test files, 254 tests, all green
- 📋 **Remaining** — real `SchedulingModule` backend integration (currently MSW-mocked); a real multi-doctor directory (today's booking flow is scoped to the single demo doctor account, by design); reminders (client-side display only, no Notification module to deliver them, per this phase's own scope line); recurring appointments and external calendar sync remain 🧊 deferred as originally scoped
- 🔒 **Blocked** — Queue Management (waiting queue, doctor queue, token system, estimated wait time, queue dashboard) remains explicitly out of scope, blocked on its own backend Queue module (Section 1.4) — never silently folded into this phase's Scheduling work, which is a distinct backend concern
