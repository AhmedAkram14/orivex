...
- Storybook (Vite-based Next.js framework — `@storybook/nextjs`'s webpack builder was tried first and hits a hard, structural incompatibility with Next.js's internally-vendored webpack copy that no version pin fixes; documented in `.storybook/main.ts`) with a story per component, an LTR/RTL and light/dark toolbar switcher, and the a11y addon set to fail on violations
