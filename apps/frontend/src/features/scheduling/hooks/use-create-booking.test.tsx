...
vi.mock('@/features/scheduling/api/scheduling-api', () => ({
  schedulingApi: { createBooking: vi.fn() },
}));
...
describe('useCreateBooking', () => {
  it('invalidates every appointment-derived cache on both dashboards after a successful booking', async () => {
    vi.mocked(schedulingApi.createBooking).mockResolvedValue({
      id: 'booking-1',
    ...
    const { result } = renderHook(() => useCreateBooking(), { wrapper: createWrapper(queryClient) });
