...
export function ConsultationContainer({ leftNav, children, rightPanel, className }: ConsultationContainerProps) {
  return (
    <div className={cn('grid grid-cols-1 gap-4 lg:grid-cols-[240px_1fr] xl:grid-cols-[240px_1fr_280px]', className)}>
      ...
      <main className="min-h-96 rounded-lg border border-border-default bg-surface p-4">{children}</main>
      {rightPanel && (
        <aside className="flex flex-col gap-3 rounded-lg border border-border-default bg-surface p-4 xl:block">
          {rightPanel}
        </aside>
      )}
    </div>
