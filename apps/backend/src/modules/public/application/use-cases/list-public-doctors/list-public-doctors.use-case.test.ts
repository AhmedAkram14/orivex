...
describe('ListPublicDoctorsUseCase', () => {
  ...
  it('joins real rating aggregates onto each doctor and sorts highest-rated first', async () => {
    ...
    const aggregates = new Map<string, DoctorRatingAggregate>([
      ['doctor-a', { averageRating: 4.2, reviewCount: 10, writtenReviewCount: 6, averageCommunicationRating: null, averagePunctualityRating: null, averageThoroughnessRating: null }],
      ['doctor-b', { averageRating: 4.8, reviewCount: 3, writtenReviewCount: 2, averageCommunicationRating: null, averagePunctualityRating: null, averageThoroughnessRating: null }],
  ...
  it('tags the highest-rated reviewed doctor as top rated, never an unreviewed one', async () => {
    ...
    const aggregates = new Map<string, DoctorRatingAggregate>([['doctor-b', { averageRating: 4.9, reviewCount: 1, writtenReviewCount: 1, averageCommunicationRating: null, averagePunctualityRating: null, averageThoroughnessRating: null }]]);
  ...
  it('tags the doctor with the most real bookings as most booked, never the same doctor already tagged top rated', async () => {
    ...
    const doctorB = buildEntry({ doctorProfileId: 'doctor-b' });
    const queryPort = new FakePublicDirectoryQueryPort({ entries: [doctorA, doctorB], total: 2 });
    const aggregates = new Map<string, DoctorRatingAggregate>([['doctor-a', { averageRating: 5, reviewCount: 1, writtenReviewCount: 1, averageCommunicationRating: null, averagePunctualityRating: null, averageThoroughnessRating: null }]]);
