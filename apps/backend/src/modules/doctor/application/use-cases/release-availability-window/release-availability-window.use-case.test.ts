...
describe('ReleaseAvailabilityWindowUseCase', () => {
  it('releases a held window back to Open', async () => {
    ...
    const result = await useCase.execute(new ReleaseAvailabilityWindowCommand({ availabilityWindowId: window.getId() }));
  ...
  it('throws NotFoundError when the window does not exist', async () => {
    ...
    await assert.rejects(
      () => useCase.execute(new ReleaseAvailabilityWindowCommand({ availabilityWindowId: 'missing-id' })),
      NotFoundError,
    );
  });

  it('propagates the domain error when the window is not held', async () => {
    const startTime = new Date(Date.now() + 60 * 60_000);
    const window = AvailabilityWindow.define({
      doctorId: '11111111-1111-4111-8111-111111111111',
      startTime,
    ...
    await assert.rejects(
      () => useCase.execute(new ReleaseAvailabilityWindowCommand({ availabilityWindowId: window.getId() })),
      DoctorDomainError,
    );
  });
});
