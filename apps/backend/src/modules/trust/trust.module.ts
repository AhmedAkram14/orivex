...
// consent scope category resolution). ConsultationModule itself doesn't
// import TrustModule, but it does import AuthenticationModule, which does
// (for RecordSecurityEventUseCase/ListSecurityEventsForAccountUseCase) --
// a real transitive cycle (TrustModule -> ConsultationModule ->
// AuthenticationModule -> TrustModule) that crashes Node's ESM loader with
// "Cannot access 'X' before initialization" the moment any entry point's
// import order resolves this edge first (confirmed: it broke both
// `dist/main.js` and the Prisma seed script). forwardRef() defers the
// class reference past module-evaluation time, which is exactly what a
// real (not just Nest-DI-level) circular import needs.
@Module({
  imports: [
    DoctorModule,
    PatientModule,
