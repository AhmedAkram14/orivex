...
// VerificationCaseNotApprovedError are checked first since both are
// TrustDomainError subtypes -- a VerificationCase state conflict is a 409,
// distinct from the generic 422 used for other domain violations in this
// module.
export function mapTrustError(error: unknown): unknown {
  if (error instanceof VerificationCaseAlreadyDecidedError || error instanceof VerificationCaseNotApprovedError) {
