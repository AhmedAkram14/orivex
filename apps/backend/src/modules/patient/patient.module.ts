...
// No controllers -- docs/12-openapi.md documents no /patients CRUD contract
// (only a lightweight PatientSummary schema used elsewhere, and unrelated
// consent/health-graph/journey sub-resources). Per architect direction, this
// stays an internal application-layer module until a formal Patient API is
// documented; its use cases are exported ahead of any consumer, same
// precedent as AssetModule's Sprint 3 use cases.
//
...
@Module({
  ...
  exports: [CreatePatientProfileUseCase, GetPatientProfileByIdUseCase, UpdatePatientProfileUseCase],
})
