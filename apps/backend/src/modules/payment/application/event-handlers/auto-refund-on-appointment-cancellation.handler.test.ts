...
import { AutoRefundOnAppointmentCancellationHandler } from './auto-refund-on-appointment-cancellation.handler.js';
...
describe('AutoRefundOnAppointmentCancellationHandler', () => {
  it("refunds the appointment's Succeeded payment transaction when the DOCTOR cancels", async () => {
    const transaction = buildSucceededTransaction('pi_cancel_doctor');
    ...
    const handler = new AutoRefundOnAppointmentCancellationHandler(repository, refundPaymentUseCase, logger as never);

    await handler.handle({ appointmentId: APPOINTMENT_ID, cancelledBy: 'doctor' });

    assert.equal(gateway.refundCallCount, 1);
    assert.equal(gateway.lastRefundRequest?.externalReference, 'pi_cancel_doctor');
    assert.equal(repository.saved.length, 1);
    assert.equal(repository.saved[0]?.getStatus(), PaymentStatus.Refunded);
    assert.equal(logger.errors.length, 0);
  });

  it("refunds the appointment's Succeeded payment transaction when the PATIENT cancels (no guard, no cutoff, always full refund)", async () => {
    const transaction = buildSucceededTransaction('pi_cancel_patient');
    const repository = new FakePaymentTransactionRepository(transaction);
    const gateway = new FakeGateway({ succeeded: true });
    const refundPaymentUseCase = new RefundPaymentUseCase(repository, gateway, new RecordingDispatcher());
    const logger = new FakeLogger();
    ...
    await handler.handle({ appointmentId: APPOINTMENT_ID, cancelledBy: 'patient' });

    assert.equal(gateway.refundCallCount, 1);
    assert.equal(gateway.lastRefundRequest?.externalReference, 'pi_cancel_patient');
    assert.equal(repository.saved.length, 1);
    assert.equal(repository.saved[0]?.getStatus(), PaymentStatus.Refunded);
    assert.equal(logger.errors.length, 0);
  });

  it('no-ops without throwing when no payment transaction exists for the appointment (free/never-charged, Requested cancellation)', async () => {
  ...
  it('safely swallows a replayed event hitting an already-Refunded transaction (no double refund, no throw)', async () => {
    const transaction = buildSucceededTransaction('pi_cancel_already');
    transaction.refund();
    transaction.releaseDomainEvents();
    const repository = new FakePaymentTransactionRepository(transaction);
    const gateway = new FakeGateway();
    ...
    // ever reaching RefundPaymentUseCase/the gateway, for either canceller.
    await handler.handle({ appointmentId: APPOINTMENT_ID, cancelledBy: 'patient' });
  ...
  it('logs and swallows a gateway failure instead of throwing (cancellation already saved, must not be corrupted)', async () => {
    const transaction = buildSucceededTransaction('pi_cancel_fails');
    const repository = new FakePaymentTransactionRepository(transaction);
