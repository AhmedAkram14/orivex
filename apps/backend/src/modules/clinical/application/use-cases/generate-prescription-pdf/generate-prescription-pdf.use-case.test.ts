...
} from '../../ports/prescription-pdf-generator.port.js';

import { GeneratePrescriptionPdfUseCase } from './generate-prescription-pdf.use-case.js';
...
class FakePrescriptionPdfGeneratorPort implements PrescriptionPdfGeneratorPort {
  public lastRequest?: GeneratePrescriptionPdfRequest;
...
function buildUseCase(
  ...
  pdfGenerator: FakePrescriptionPdfGeneratorPort,
): GeneratePrescriptionPdfUseCase {
  return new GeneratePrescriptionPdfUseCase(
...
describe('GeneratePrescriptionPdfUseCase', () => {
  it('composes real doctor/patient identity and medications, then delegates to the PDF port', async () => {
    const scenario = buildScenario();
    const pdfGenerator = new FakePrescriptionPdfGeneratorPort();
    const useCase = buildUseCase(scenario, scenario.prescription, pdfGenerator);

    const pdf = await useCase.execute({
      prescriptionId: scenario.prescription.getId(),
      verificationUrl: 'https://example.com/verify-prescription/TEST-CODE',
    });

    assert.ok(Buffer.isBuffer(pdf));
    assert.equal(pdfGenerator.lastRequest?.doctorName, 'Dr. Sarah Ahmed');
    assert.equal(pdfGenerator.lastRequest?.doctorLicenseNumber, 'LIC-9001');
    assert.equal(pdfGenerator.lastRequest?.patientName, 'Amina Youssef');
    assert.equal(pdfGenerator.lastRequest?.verificationCode, 'TEST-CODE');
    assert.equal(pdfGenerator.lastRequest?.lineItems[0]?.drugName, 'Amoxicillin');
  });

  it('throws NotFoundError when the prescription does not exist', async () => {
    const scenario = buildScenario();
    const useCase = buildUseCase(scenario, null, new FakePrescriptionPdfGeneratorPort());

    await assert.rejects(
      () => useCase.execute({ prescriptionId: 'missing-id', verificationUrl: 'https://example.com/verify/x' }),
      NotFoundError,
    );
  });

  it('throws ValidationError for an unsigned prescription (no signature/verification code)', async () => {
    const scenario = buildScenario();
    const unsigned = Prescription.reconstitute({
      id: scenario.prescription.getId(),
      consultationSessionId: scenario.prescription.getConsultationSessionId(),
      diagnosisNodeId: scenario.prescription.getDiagnosisNodeId(),
      authoringDoctorId: scenario.prescription.getAuthoringDoctorId(),
      status: scenario.prescription.getStatus(),
      lineItems: scenario.prescription.getLineItems(),
      createdAt: scenario.prescription.getCreatedAt(),
      updatedAt: scenario.prescription.getUpdatedAt(),
    });
    const useCase = buildUseCase(scenario, unsigned, new FakePrescriptionPdfGeneratorPort());

    await assert.rejects(
      () => useCase.execute({ prescriptionId: unsigned.getId(), verificationUrl: 'https://example.com/verify/x' }),
      ValidationError,
    );
  });
});
