...
import { GrantConsentCommand } from '../../../trust/application/use-cases/grant-consent/grant-consent.command.js';
import { GrantConsentUseCase } from '../../../trust/application/use-cases/grant-consent/grant-consent.use-case.js';
...
class EmptyHealthJourneyRepository implements HealthJourneyRepository {
  async findById() {
    return null;
  }
  async findByHealthGraphId() {
    return [];
  }
  async save(): Promise<void> {}
}
...
// Guard-gap fix (this audit's P0 C1): proves a Doctor can only read a
// patient's health graph/journeys when they have a real treating
// relationship with that patient -- the same DOCTOR-OWNED ENCOUNTERS ONLY
// primitive DoctorPatientChartController already uses. Before this fix,
// ensureReadableByCaller returned unconditionally for any Doctor role.
describe('HealthGraphController (integration)', () => {
  ...
  let getConsentStateUseCase: GetConsentStateUseCase;
  let grantConsentUseCase: GrantConsentUseCase;
  ...
  before(async () => {
    ...
    const otherPatientAccount = Account.register({
      email: EmailAddress.create('other-patient@example.com'),
      role: AccountRole.Patient,
      displayName: DisplayName.create('Yara Adel'),
    });
    otherPatient = PatientProfile.create({ accountId: otherPatientAccount.getId().toString() });
    ...
    // patient. Before the fix, this doctor could still read the patient's
    // full health graph.
    ...
    conditionNodeId = healthGraph.getNodes()[0].getId();
    ...
    getConsentStateUseCase = new GetConsentStateUseCase(consentRecordRepository, getConsentScopeCategoryByCodeUseCase);
    grantConsentUseCase = new GrantConsentUseCase(
      consentRecordRepository,
      getConsentScopeCategoryByCodeUseCase,
      new NoopDomainEventDispatcher(),
    );
    revokeConsentUseCase = new RevokeConsentUseCase(
      consentRecordRepository,
      getConsentScopeCategoryByCodeUseCase,
      new NoopDomainEventDispatcher(),
    );
    ...
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        RolesGuard,
        {
          ...
          useFactory: () =>
            new FakeJwtSigner({
              ...
              [OTHER_PATIENT_TOKEN]: { accountId: otherPatientAccount.getId().toString(), role: AccountRole.Patient },
            }),
        },
        {
        ...
        },
        {
          ...
          useFactory: () =>
            new ListHealthJourneysUseCase(healthGraphRepository, new EmptyHealthJourneyRepository(), new GetPatientProfileByIdUseCase(patientProfileRepository)),
        },
        {
  ...
  it('a doctor with a real appointment with the patient gets 200 with the real health graph and an audit row is recorded', async () => {
    ...
    assert.equal(response.body.data[0].id, conditionNodeId);

    // Audit trail gap fix (ORIVEX Remaining Work Audit, P0 C2): a real,
    // successful PHI read must leave a real audit row.
    const entry = auditLogRepository.recorded.find((e) => e.getAction() === 'health_graph_read');
    assert.ok(entry, 'expected an audit log entry for this health graph read');
    assert.equal(entry?.getActorAccountId(), doctorA.getAccountId());
    assert.equal(entry?.getActorRole(), 'doctor');
    assert.equal(entry?.getSubjectId(), patient.getId());
  });

  it('a doctor with NO relationship to the patient gets an ownership-safe not-found on the health graph, not the full record, and no audit row is recorded', async () => {
    const auditCountBefore = auditLogRepository.recorded.length;
    const response = await request(app.getHttpServer())
      .get(`/patients/${patient.getId()}/health-graph`)
    ...
    // A rejected read is not an access -- it must never be logged as one.
    assert.equal(auditLogRepository.recorded.length, auditCountBefore);
  });

  it('a doctor with NO relationship to the patient gets not-found on journeys too', async () => {
    const response = await request(app.getHttpServer())
  ...
  it('a doctor WITH a relationship still succeeds on journeys (no regression from the fix)', async () => {
    const response = await request(app.getHttpServer())
      .get(`/patients/${patient.getId()}/journeys`)
      .set('Authorization', `Bearer ${DOCTOR_A_TOKEN}`)
      .expect(200);
  ...
  it('a patient can read their own health graph', async () => {
    const response = await request(app.getHttpServer())
      .get(`/patients/${patient.getId()}/health-graph`)
  ...
  it("a patient cannot read another patient's health graph", async () => {
    const response = await request(app.getHttpServer())
      .get(`/patients/${patient.getId()}/health-graph`)
  ...
  // ownership-safe 404 used for "no relationship at all."
  describe('consent revocation (P0 C3)', () => {
    it('blocks the treating doctor with 403 CONSENT_NOT_GRANTED once the patient revokes consent, then restores access on re-grant', async () => {
      ...
      // A patient's own read is never affected by a doctor-scoped revoke.
