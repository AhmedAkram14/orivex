...
import { NotifyDoctorOfAppointmentConfirmedHandler } from './notify-doctor-of-appointment-confirmed.handler.js';
...
function buildAppointment(patientId: string, doctorId: string, scheduledAt: Date): Appointment {
...
describe('NotifyDoctorOfAppointmentConfirmedHandler', () => {
  it("notifies the doctor's own account that the patient is now waiting, for an appointment scheduled today", async () => {
    ...
    // An hour from now -- safely "today" in UTC terms for a normal test run.
    const appointment = buildAppointment(patientProfile.getId(), doctorProfile.getId(), new Date(Date.now() + 60 * 60_000));
  ...
  it('is a silent no-op for an appointment confirmed for a future day -- it will not appear in the queue today, so a "patient is waiting" cue would be false', async () => {
    const doctorAccount = buildAccount(AccountRole.Doctor, 'Dr. Sarah Ahmed');
    const patientAccount = buildAccount(AccountRole.Patient, 'Amina Youssef');
    const doctorProfile = DoctorProfile.register({
      accountId: doctorAccount.getId().toString(),
