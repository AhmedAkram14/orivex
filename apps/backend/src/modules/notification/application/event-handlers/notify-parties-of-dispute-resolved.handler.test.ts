...
import { NotifyPartiesOfDisputeResolvedHandler } from './notify-parties-of-dispute-resolved.handler.js';
...
function buildHandler(props: {
  ...
  return new NotifyPartiesOfDisputeResolvedHandler(
...
describe('NotifyPartiesOfDisputeResolvedHandler', () => {
  it('notifies both the raiser and the counterparty, and emails both', async () => {
    ...
    dispute.resolve(DisputeStatus.Resolved, 'Confirmed via session logs.', 'admin-account');
    ...
    assert.ok(notificationRepo.saved.every((notification) => notification.getTitle() === 'Dispute resolved'));
    // PHI-light: the admin's actual resolution notes never leak into the copy.
    assert.ok(notificationRepo.saved.every((notification) => !notification.getDescription().includes('session logs')));

    const raiserNotification = notificationRepo.saved.find((n) => n.getAccountId() === patient.getAccountId())!;
    assert.match(raiserNotification.getDescription(), /Your dispute/);
    const counterpartyNotification = notificationRepo.saved.find((n) => n.getAccountId() === doctor.getAccountId())!;
    assert.match(counterpartyNotification.getDescription(), /one of your appointments/);

    assert.equal(emailSender.sent.length, 2);
    assert.ok(emailSender.sent.every((e) => e.template === 'dispute-resolved'));
