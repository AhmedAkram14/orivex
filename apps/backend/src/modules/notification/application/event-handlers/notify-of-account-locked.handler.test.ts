...
import { NotifyOfAccountLockedHandler } from './notify-of-account-locked.handler.js';
...
describe('NotifyOfAccountLockedHandler', () => {
  it('notifies the account of its own lockout, including when it expires', async () => {
    const notificationRepo = new FakeNotificationRepository();
    const logger = new FakeLogger();
    const handler = new NotifyOfAccountLockedHandler(notificationRepo, logger as never);
    const lockedUntil = new Date(Date.now() + 15 * 60_000);

    await handler.handle({ accountId: ACCOUNT_ID, lockedUntil });
    ...
    assert.equal(notification.getTitle(), 'Account temporarily locked');
  ...
  it('logs (and never throws) when saving fails', async () => {
    ...
    await handler.handle({ accountId: ACCOUNT_ID, lockedUntil: new Date() });
