...
import { NotifyPatientOfAppointmentExpiredHandler } from './notify-patient-of-appointment-expired.handler.js';
...
describe('NotifyPatientOfAppointmentExpiredHandler', () => {
  it('notifies the patient that their request expired unanswered, and emails them', async () => {
    ...
    assert.equal(notification.getTitle(), 'Appointment request expired');
    assert.match(notification.getDescription(), /expired unanswered/);
    assert.equal(notification.getActionUrl(), '/patient/appointments');
    assert.equal(logger.errors.length, 0);

    assert.equal(emailSender.sent.length, 1);
    assert.equal(emailSender.sent[0]!.template, 'appointment-expired');
  });
