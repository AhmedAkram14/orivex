...
import { NotifyCounterpartyOfDisputeRaisedHandler } from './notify-counterparty-of-dispute-raised.handler.js';
...
function buildHandler(props: {
  ...
  return new NotifyCounterpartyOfDisputeRaisedHandler(
...
describe('NotifyCounterpartyOfDisputeRaisedHandler', () => {
  it('notifies the doctor when the patient raised the dispute, and emails them -- never the raiser', async () => {
    ...
    assert.match(notification.getDescription(), /A dispute was raised/);
    assert.doesNotMatch(notification.getDescription(), /no-show|fault|wrong/i);
    assert.equal(notification.getActionUrl(), '/doctor/disputes');

    assert.equal(emailSender.sent.length, 1);
    assert.equal(emailSender.sent[0]!.template, 'dispute-raised');
    assert.equal(logger.errors.length, 0);
  });

  it('notifies the patient when the doctor raised the dispute', async () => {
    const { patientAccount, doctorAccount, patient, doctor, appointment } = buildScenario();
    const dispute = Dispute.raise({
      appointmentId: appointment.getId(),
    ...
    assert.equal(notification.getActionUrl(), '/patient/disputes');
  });
