...
import { NotifyPatientOfRefundIssuedHandler } from './notify-patient-of-refund-issued.handler.js';
...
function buildRefundedTransaction(patientId: string): PaymentTransaction {
  ...
  transaction.attachExternalReference('pi_123');
  transaction.refund();
  return transaction;
}

describe('NotifyPatientOfRefundIssuedHandler', () => {
  it("notifies the patient's own account with the refunded amount", async () => {
    const patient = PatientProfile.create({ accountId: '55555555-5555-4555-8555-555555555555' });
    const transaction = buildRefundedTransaction(patient.getId());
    ...
    assert.equal(notification.getTitle(), 'Refund issued');
