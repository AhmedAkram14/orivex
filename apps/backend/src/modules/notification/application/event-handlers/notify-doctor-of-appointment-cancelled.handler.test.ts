...
import { NotifyDoctorOfAppointmentCancelledHandler } from './notify-doctor-of-appointment-cancelled.handler.js';
...
function buildAppointmentAndDoctor() {
  ...
  return { doctorProfile, appointment };
}

describe('NotifyDoctorOfAppointmentCancelledHandler', () => {
  it('notifies the doctor when the patient cancels', async () => {
    const { doctorProfile, appointment } = buildAppointmentAndDoctor();
  ...
  it('never self-notifies the doctor when the doctor cancels their own appointment', async () => {
    const { doctorProfile, appointment } = buildAppointmentAndDoctor();
    const notificationRepo = new FakeNotificationRepository();
    const logger = new FakeLogger();
    const handler = new NotifyDoctorOfAppointmentCancelledHandler(
  ...
  it('is a silent no-op for an unknown appointment id (never throws)', async () => {
    ...
    await handler.handle({ appointmentId: '99999999-9999-4999-8999-999999999999', cancelledBy: 'patient' });
