...
import { NotifyPartiesOfDisputeDismissedHandler } from './notify-parties-of-dispute-dismissed.handler.js';
...
function buildHandler(props: {
  ...
  return new NotifyPartiesOfDisputeDismissedHandler(
...
describe('NotifyPartiesOfDisputeDismissedHandler', () => {
  it('notifies both the raiser and the counterparty, and emails both', async () => {
    ...
    dispute.resolve(DisputeStatus.Dismissed, 'No evidence found.', 'admin-account');
    ...
    assert.ok(notificationRepo.saved.every((notification) => notification.getTitle() === 'Dispute dismissed'));
    assert.ok(notificationRepo.saved.every((notification) => !notification.getDescription().includes('No evidence found')));

    const raiserNotification = notificationRepo.saved.find((n) => n.getAccountId() === doctor.getAccountId())!;
    assert.match(raiserNotification.getDescription(), /Your dispute/);

    assert.equal(emailSender.sent.length, 2);
    assert.ok(emailSender.sent.every((e) => e.template === 'dispute-dismissed'));
